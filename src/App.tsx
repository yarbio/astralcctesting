...
function FeatureCard(props: { title: string, description: string }) {
  return (
    <div className='border-[2px] m-2 max-w-[23rem] w-[23rem] rounded-xl'>
      <div className='bg-transparent m-4 max-h-60 h-60'>
        <div className='bg-transparent'>
          <h3 className='bg-transparent font-sg font-semibold text-xl text-center'>{props.title}</h3>
        </div>
        <p className='text-center font-sg bg-transparent'>{props.description}</p>
      </div>
    </div>
  )
}

function TierCard(props: { title: string, preferred: boolean, description: string, price: string }) {
  return (
    props.preferred ?
  ...
  return (
    <>
      ...
      <div id='home' className='flex max-w-[100%] w-[100vw] h-[70%] justify-center'>
        <div className='mt-[15vh] w-[60%]'>
          ...
          <div id='pricing' className='mt-20'>
            ...
            <div className='flex flex-wrap justify-center overflow-hidden'>
              <TierCard title='week' preferred={false} price='5.98' description="Great for a quick test drive of all features with no long-term commitment." />
              <TierCard title='month' preferred={true} price='8.98' description='Enjoy full access for a month—perfect for ongoing, flexible use.' />
              <TierCard title='lifetime' preferred={false} price='20.00' description='Enjoy all features with no limits, no subscriptions—just endless access.' />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className='flex max-w-[100%] pb-[12vh] mt-[5vh] justify-center bg-[#222222]'>
        <div className='w-[70%] my-5 bg-transparent'>
          <div className='flex flex-col bg-transparent'>
            <a className='text-[#4d4d4d]' target='_BLANK' href='https://discord.gg/GGnjTSxMh2'>Discord</a>
            <a className='text-[#4d4d4d]' target='_BLANK' href='https://github.com/raycast6000/astralis-web'>Source code</a>
            <span className='font-sg'>© 2024 Astralis</span>
          </div>
        </div>
      </div>
    </>
